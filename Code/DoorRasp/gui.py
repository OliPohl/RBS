...
class Win(tk.Tk):
    ...
    def StartScanIdThread(self):
        ...
        self.timeEnd = datetime.now() + timedelta(hours=2) + timedelta(seconds=10)
    ...
    def ScanId(self, route: int):
        ...
        if self.rfidManager.userId != None:
            ...
            elif datetime.now() + timedelta(hours=2) >= self.timeEnd:
    ...
    def LoginEntry(self):
        print("Login Entry")
        self.databaseHandler.AddEntry(self.userId, datetime.now() + timedelta(hours=2), datetime.now() + timedelta(hours=2) + timedelta(minutes=self.duration))
    ...
    def ConfirmSettings(self):
        ...
        if self.blockDuration > 0:
            ...
            self.databaseHandler.AddEntry(self.userId, datetime.now() + timedelta(hours=2), datetime.now() + timedelta(hours=2) + timedelta(minutes=self.blockDuration))

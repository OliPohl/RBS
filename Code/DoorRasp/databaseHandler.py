...
class DatabaseHandler: 
    ...
    def DeleteExpiredEntries(self):
        ...
        for entry in results:
            if datetime.now() + timedelta(hours=2) >= entry["exitTime"]:
                self.DeleteEntry(entry["userId"])
...
def main():
    ...
    databaseHandler.AddEntry("John1232", datetime.now() + timedelta(hours=2), datetime.now() + timedelta(hours=2) + timedelta(minutes=15))
    databaseHandler.AddEntry("Tom1232", datetime.now() + timedelta(hours=2), datetime.now() + timedelta(hours=2) + timedelta(minutes=30))
    databaseHandler.AddEntry("jimmy", datetime.now() + timedelta(hours=2), datetime.now() + timedelta(hours=2) + timedelta(minutes=450))
